...
- **Phase 4 — New features + JIT** *(done)*: built-in profiler (`/neoskript profile`); scheduled `every <timespan>:` triggers and `on load:`; Folia-safe repeating scheduler; `random number/integer` expression; pluggable `ExecutionEngine` with hot-path detection (the seam for a future ASM bytecode backend — interpreter remains the engine today). *Actual bytecode emission and continuation-based delays/async are deferred to keep correctness.*
- **Phase 5 — Polish** *(done)*: ServiceLoader addon discovery + programmatic `registerAddon` (the `NeoSkriptAddon` SPI is now live); a `DocsGenerator` that renders a syntax reference from the live registries (`/neoskript docs`); curated [`SYNTAX.md`](SYNTAX.md) and [`CONTRIBUTING.md`](../CONTRIBUTING.md). *A hosted docs site and public beta are release activities beyond the codebase.*
